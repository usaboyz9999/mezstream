...
// ─── Player Screen ─── (moved to screens/PlayerScreen.js) ────────────────────
// kept as stub for backward compatibility — real player is in PlayerScreen.js
...
export function AccountScreen({ navigation }) {
  const { colors: C, isRTL, currentUser, login, register, logout, showModal, t, toggleDarkMode, toggleLanguage, isDark, language, watchHistory, clearHistory } = useApp();
    ...
    <ScrollView style={{ flex: 1, backgroundColor: C.bg }} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ padding: 16, gap: 10 }}>
        ...
        {[
          ...
          { icon: isDark ? 'sunny-outline' : 'moon-outline', label: isDark ? 'الوضع الفاتح' : 'الوضع الداكن', action: () => toggleDarkMode() },
          { icon: 'language-outline', label: language === 'ar' ? 'English' : 'عربي', action: () => toggleLanguage() },
